...
type WorkingHour = { day: string; start: string | null; end: string | null; dayOff: boolean };
type ServiceRow = { id: string; name: string; emoji: string };
type StaffRow = {
...
export function StaffClient({ initialStaff, serviceList }: { initialStaff: StaffRow[]; serviceList: ServiceRow[] }) {
  ...
  const handleToggleActive = (s: StaffRow) => {
  ...
  const handleSaveEdit = () => {
    ...
    startTransition(async () => {
      await updateStaff(editing.id, {
        ...
        workingHours: editing.workingHours.map((h) => ({
          day: h.day,
  ...
  const toggleService = (svcId: string) => {
    if (!editing) return;
    const has = editing.serviceIds.includes(svcId);
    setEditing({ ...editing, serviceIds: has ? editing.serviceIds.filter((x) => x !== svcId) : [...editing.serviceIds, svcId] });
  };
    ...
    <div className="p-4 md:p-8">
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "#B08D7A" }}>{initialStaff.filter((s) => s.isActive).length} aktif uzman</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
        {initialStaff.map((s) => {
          const svcList = serviceList.filter((svc) => s.serviceIds.includes(svc.id));
          return (
            <div key={s.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-opacity ${s.isActive ? "border-[#E8E0DA]" : "border-[#F0EAE5] opacity-60"}`}>
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#F5F0EC]">
                  <span className="text-xs text-[#9A8A82]">{s.appointmentCount} toplam randevu</span>
                  <button
                ...
                <div className="flex flex-wrap gap-2">
                  {serviceList.map((svc) => (
                    <button key={svc.id}
                      ...
                      className={`text-xs px-3 py-1.5 rounded-full border transition-all ${editing.serviceIds.includes(svc.id) ? "text-white border-transparent" : "border-[#E8E0DA] text-[#7A6A62]"}`}
                      style={editing.serviceIds.includes(svc.id) ? { background: "#B08D7A" } : {}}>
                      {svc.emoji} {svc.name}
                    </button>
                  ))}
                </div>
