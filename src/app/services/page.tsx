...
import { services as initialServices, staff } from "@/lib/mock-data";
...
export default function ServicesPage() {
  ...
  const deleteSvc = (id: string) => setServices((prev) => prev.filter((s) => s.id !== id));
  ...
  const toggleStaff = (arr: string[], id: string) =>
    ...
    <div className="p-4 md:p-8">
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "#B08D7A" }}>{services.filter((s) => s.isActive).length} aktif hizmet</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {services.map((svc) => {
          const svcStaff = staff.filter((s) => svc.staffIds.includes(s.id));
          return (
            <div key={svc.id} className={`bg-white rounded-2xl border shadow-sm p-5 transition-opacity ${svc.isActive ? "border-[#E8E0DA] opacity-100" : "border-[#F0EAE5] opacity-60"}`}>
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => deleteSvc(svc.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 transition-colors">
            ...
            <div className="space-y-3 mt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="number" className="mt-1 w-full px-3 py-2 text-sm bg-[#FAF7F4] border border-[#E8E0DA] rounded-xl outline-none focus:border-[#B08D7A]"
                    value={editing.price} onChange={(e) => setEditing({ ...editing, price: parseFloat(e.target.value) })} />
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {staff.map((s) => (
                    <button
                      key={s.id}
                      onClick={() => setEditing({ ...editing, staffIds: toggleStaff(editing.staffIds, s.id) })}
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent className="bg-white rounded-2xl border-[#E8E0DA] mx-4 md:mx-auto">
          ...
          <div className="space-y-3 mt-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {staff.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => setNewSvc({ ...newSvc, staffIds: toggleStaff(newSvc.staffIds, s.id) })}
            ...
            <div className="flex gap-2 pt-2">
              <Button onClick={addSvc} className="flex-1 text-white" style={{ background: "linear-gradient(135deg, #B08D7A, #C9A898)", border: "none" }}>Kaydet</Button>
